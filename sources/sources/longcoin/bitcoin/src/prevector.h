...
Внедряет замену для std :: vector <T>, которая хранит до N элементов напрямую (без выделения кучи). Типы Size и Diff используются для хранения счетчиков элементов и могут быть любыми неподписанными + подписанными типами.

 * Схема размещения:
 * - Прямое распределение:
 *   - Размер _size: количество используемых элементов (между 0 и N)
 *   - T direct [N]: массив из N элементов типа T
 *     (инициализируется только первый _size).
 * - Косвенное распределение:
 *   - Размер _size: количество используемых элементов плюс N + 1
 *   - Размерная емкость: количество выделенных элементов
 *   - T * косвенный: указатель на массив элементов емкости типа T
 *    (инициализируется только первый _size).

 Тип данных T должен быть перемещен с помощью memmove / realloc (). Как только мы перейдем на C ++, вместо этого можно использовать конструкторы.
